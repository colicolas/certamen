      ...
      <ul className="list-none border-l border-gray-300 pl-4">
        {headers.map(header => (
          <li key={uuidv4()} className={`ml-${(header.level - 1) * 4} mb-2 relative`}>
            ...
            <Link href={`/study/${(division as string).toUpperCase()}/${(category as string).toLowerCase()}/${lesson}#${header.id}`} className="transition duration-300 text-gray-800 hover:text-gray-600 hover:underline">
              {header.text}
            </Link>
          </li>
        ))}
  ...
  return (
    <nav>
      ...
      <ul className="list-none">
        {headers.map(header => (
          <li key={uuidv4()} className={`ml-${(header.level - 1) * 4}`}>
            <Link href={`/study/${division}/${category}/${lesson}#${header.id}`}>
              {header.text}
            </Link>
          </li>
        ))}
